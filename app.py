...
# Estilo CSS con la animación "Arriba y Abajo"
st.markdown("""
    <style>
...
# ==========================================
# 🔍 BUSCADOR DE INFORMACIÓN Y PERSONAJES
# ==========================================
def buscar_informacion_web(consulta):
    """Busca descripciones de personajes o temas en Wikipedia."""
    try:
...
# ==========================================
# 💾 BARRA LATERAL CON OPCIÓN DE BORRAR CHAT
# ==========================================
with st.sidebar:
...
# 📷 ADJUNTAR IMÁGENES
with st.expander("📷 Adjuntar Imagen de Referencia o Archivo", expanded=False):
    imagen_subida = st.file_uploader("Sube una imagen para analizar:", type=["png", "jpg", "jpeg", "webp"])
    if imagen_subida:
...
if not bloqueado_por_restriccion:
    if prompt := st.chat_input("Pídeme un código, escribe 'dibuja a Tanjiro' o haz una pregunta..."):
        ...
        with st.chat_message("assistant"):
            ...
            # 🎬 ANIMACIÓN Y CLASIFICACIÓN DE INTENCIÓN
            marcador_animado.markdown(
                '<div class="animacion-cargando">🧠 Clasificando consulta (Pregunta, Código, Imagen o Ayuda)...</div>', 
                unsafe_allow_html=True
            )
            time.sleep(1.0) # Tiempo deliberado para clasificar
            ...
            exclusiones = ["codigo", "código", "programa", "python", "escribe", "explica", "resuelve"]

            quiere_imagen = any(p in texto_lower for p in palabras_creacion) and not any(e in texto_lower for e in exclusiones)

            # 🎨 OPCIÓN 1: GENERAR IMAGEN CON BÚSQUEDA WEB PREVIA
            if quiere_imagen:
                marcador_animado.markdown(
                    '<div class="animacion-cargando">🔍 Buscando el personaje en la web para ver cómo es...</div>', 
                    unsafe_allow_html=True
                )
                ...
                # Buscar apariencia en Wikipedia
                ...
                try:
                    ...
                    urllib.request.urlopen(url_final_imagen, timeout=30)
            ...
            # 📝 OPCIÓN 2: TEXTO / CÓDIGO (ESCRITURA LENTA LETRA POR LETRA)
            else:
                ahora = datetime.now()
                contexto_sistema = f"Eres Isaac AI, una IA avanzada y directa. Fecha actual: {ahora.strftime('%d/%m/%Y')}"
                
                # 🛠️ CORRECCIÓN CLAVE: LIMPIEZA DEL HISTORIAL PARA EVITAR ERROR 400
                mensajes_api = [{"role": "system", "content": contexto_sistema}]
                
                for m in chat_activo["historial"]:
                    role = m.get("role")
                    content_texto = f'Imagen generada sobre: "{m.get("prompt","")}"' if m.get("tipo") == "imagen" else m.get("content")

                    if not content_texto or not isinstance(content_texto, str):
                        continue
                ...
                if imagen_subida is not None:
                    try:
                        ...
                        st.warning(f"Error cargando imagen de referencia: {e}")
                ...
                try:
                    ...
                    # ⏱️ EFECTO MÁQUINA DE ESCRIBIR LETRA POR LETRA
                    for parte in stream:
                        ...
                        if contenido:
                            for letra in contenido:
                                ...
                                time.sleep(0.03) # Control de velocidad letra por letra
